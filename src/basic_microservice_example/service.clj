(ns basic-microservice-example.service
  (:require [io.pedestal.http :as http]
            ...
            [basic-microservice-example.controllers.account :as controller]
...
(defn get-boletos
  [{{:keys [account-id]}     :path-params
    {:keys [boleto-storage]} :components}]
  (ring-resp/response
    (controller/get-account account-id boleto-storage)))

(defn with-middleware
  [func]
  (conj common-interceptors func))

(def routes
  #{["/" :get (conj common-interceptors `home-page)]
    ["/boletos" :get (with-middleware get-boletos)]
